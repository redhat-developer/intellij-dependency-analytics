...
public class AnalyticsReportEditorTabTitleProvider implements EditorTabTitleProvider {
    @Override
    public @NotNull String getEditorTabTitle(@NotNull Project project, @NotNull VirtualFile file) {
        // Check if file opened in Editor is analytics report, if Yes then change the title of Custom Editor Tab
        // This won't rename the physical file, hence physical temp file will still remain unique.
        String tabName = file.getName();
        if ("ar".equals(file.getExtension())) {
            try {
                ...
                if (manifestDetails.get("showParent").getAsBoolean()) {
                    tabName = "Analytics Report for " + manifestDetails.get("manifestFileParent").getAsString() + "/" + manifestDetails.get("manifestName").getAsString();
                } else {
                    tabName = "Analytics Report for " + manifestDetails.get("manifestName").getAsString();
                }
            } catch (IOException e) {
